...
data = importdata(fullfile(scriptDir, "fig09_torsional_bandgap_data.txt"));
x = data(:,1);
y1 = data(:,2);
y2 = data(:,3);

%% Interpolate
% Apply shape-preserving interpolation and calculate the relative bandgap width.
...
%% Plot
% Show the bandgap boundaries on the left axis and normalized width on the right axis.
yyaxis left
...
legend([h1 h2 h3], {'Lower boundary','Upper boundary','Relative bandgap width'}, 'Location','northeast')
...
%% Export
saveas(gcf, fullfile(scriptDir, 'fig09_torsional_bandgap_vs_gamma.svg'));
